...
import {Observable, throwError, map, catchError, forkJoin} from "rxjs";
...
export class StudentsService {
   ...
   getStudentsByIds(studentIds: string[]): Observable<Student[]> {
      ...
      const studentRequests = studentIds.map(id => this.getStudentById(id));
      
      return forkJoin(studentRequests).pipe(
        catchError(error => {
          return throwError(() => new Error('Error al obtener los estudiantes'));
        })
      );
    }
}
